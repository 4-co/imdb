...
We chose to include different document types in the same container for simplicity (and to demonstrate). You can read about some of the tradeoffs [here](https://docs.microsoft.com/en-us/azure/cosmos-db/modeling-data) and see some of the side effects in the queries below. Note that some frameworks (like Spring Boot Repository) require a separate container for each document type.
...
In order to use the Cosmos DB API to read a single 1K document using 1 RU, you need to know the partition key, so having a value that you can compute the partition key from is a best practice. Note that some frameworks (like Spring Boot Repository) don't support the single document read API and always use the query API. This can have a significant cost impact depending on the access pattern.
